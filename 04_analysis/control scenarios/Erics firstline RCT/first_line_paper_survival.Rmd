...
ggsurvplot(
  ...
  title      = "Survival curve for PFS, Control arm",
...
save(fitted_PFS_chemo, file="fitted_PFS_control_firstline.RData")
```
...
ggsurvplot(
  ...
  ggtheme    = theme_bw(),       # Change ggplot2 theme
  xlab       = 'Time in months', # Change X-axis label
  title      = "Survival curve for PFS, Targeted arm",
  subtitle   = "Based on Kaplan-Meier estimates"
)
```

```{r}
surv_data$Time <- surv_data$Time/12
# times <- seq(0,5,by=1/12)
times <- 0:80
mods <- c("exp", "weibull", "gamma", "lnorm", "llogis", "gompertz" ,"rps")
fit_PFS  <- fit.fun(time = "time", status = "event", data = IPD_PFS, times = times,
...
save(fitted_PFS_targeted, file="fitted_PFS_targeted_firstline.RData")
```
