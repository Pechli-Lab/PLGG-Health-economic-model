...
df <- read_csv("PLGG_CEA_ChemoORRad_Control_Oct2023.csv")
df$status <- ifelse(df$secondprogression  == 1 | df$death == 1, 1 ,0)
df$time <- df$TimetoOutcome_from1stprogression_days/365 # change time in days to in years
```
...
ggsurvplot(
  ...
  title      = "Kaplan-Meier survival curve for PFS, Both arms",
  subtitle   = "Source: SickKids"
)
```
...
fit_control <- fit.fun(time="time", status="status", mods=mods, 
                       data=df %>% as.data.frame(), extrapolate = T, times=times) 
a <- fit_control$plot
a$plot <- a$plot + labs(subtitle="Control arm (n=18)") + scale_x_continuous(breaks=seq(0,80,10))
a$plot
```
...
fit_control <- fit.fun(time="time", status="status", mods=mods, 
                       data=df %>% as.data.frame(), extrapolate = T, times=times) 
a1 <- fit_control$plot
a1$plot <- a1$plot + labs(subtitle="Control arm (n=18), best-fitting curve") + 
...
for (i in 1:N_sim) {
  ...
  fit_PFS <- fit.fun(time  = "time", status = "status", 
                     ...
                     extrapolate = T, times=times, mods=c("exp"))
  fitted_PFS_chemo_sk[[i]] <- fit_PFS$model.objects$models$Exponential
  print(i)
}
save(fitted_PFS_chemo_sk, file="fitted_PFS_control_sk_exp_100.RData")
```
...
<!-- # df <- read_csv("plgg_cea_controls.csv") -->
<!-- df <- read_csv("PLGG_CEA_ChemoORRad_Control_Oct2023.csv") -->
<!-- df$status <- ifelse(df$death == 1, 1 ,0) -->
<!-- df$time <- df$TimetoOutcome_from1stprogression_days/365 # change time in days to in years -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- times <- 0:80 -->
<!-- mods <- c("exp", "weibull", "gamma", "llogis", "rps") -->
<!-- fit_control <- fit.fun(time="time", status="status", mods=mods,  -->
<!--                        data=df %>% as.data.frame(), extrapolate = T, times=times)  -->
<!-- a <- fit_control$plot -->
<!-- a$plot <- a$plot + labs(subtitle="Control arm (n=18)") + scale_x_continuous(breaks=seq(0,80,10)) -->
...
#                      data = PFS_data_boot %>% as.data.frame(),
#                      extrapolate = T, times=times, mods=c("exp"))
#   fitted_PFS_death[[i]] <- fit_PFS$model.objects$models$Exponential
#   print(i)
# }
# save(fitted_PFS_death, file="fitted_PFS_death_sk.RData")
```
