...
loc_here_out <- paste0(here::here(),"/04_analysis/output/scenarios/", scen,"/basecase exp exp lifetime threediscounting combo/")
...
for(sim_num1 in torun1){
  ...
  qr1 <- Microsimulation(n.i              = n.i_g,
                         ...
                         util_input       = Util_i, 
                         uindx            = 1, 
                         d.c              = 0.03/12, # 0/12 = 0% annual discount rate, to monthly
                         d.e              = 0.03/12,
                         sim_numi         = sim_num, 
                         rri              = 0,
  ...
  for( i in seq_along(qr1)){
    ...
    #saveRDS(qr1[[i]], paste0(loc_here_out, "scenarios/", scen, "/basecase exp lifetime investigator combo/", names(qr1)[i], "/", "model_", sim_num, "_RR_0.RDS" ))
  }
