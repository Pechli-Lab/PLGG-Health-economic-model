...
```{r}
fileloc <- here::here("04_analysis","output","scenarios","basecase exp lifetime investigator combo")
index <- c(1:50)
nn <- length(index)
```
...
tb2a$Variable <- factor(tb2a$Variable, levels = c("Life-years","QALY","Total Cost","Non-Targeted PLGG related costs",
                                                  "AE related costs" ,"General population costs", 
...
ggplot(data = prop_long, aes(x = cycle, y = proportion, color = state, group = state)) +
  geom_line() +
  labs(title = "Cumulative Incidence of PLGG-related AE States Across Cycles",
       x = "Year",
       y = "Proportion",
...
ggplot(data = prop_long, aes(x = cycle, y = proportion, color = state, group = state)) +
  geom_line() +
  labs(title = "Cumulative Incidence of Other AE States Across Cycles",
       x = "Year",
       y = "Proportion",
...
ggplot(p_surv1, aes(x = cycle)) +
  ...
  labs(x = "Time", y = "Survival Probability", 
       title = "Overall Survival Over Time with 95% Confidence Interval") +
...
ggplot(p_surv_plgg1, aes(x = cycle)) +
  geom_ribbon(aes(ymin = lb, ymax = ub), fill = "blue", alpha = 0.2) +  
  geom_line(aes(y = avg), color = "blue") +  
  labs(x = "Time", y = "surv_plggival Probability", 
       title = "PLGG-related survival Over Time with 95% Confidence Interval") +
  theme_bw() + facet_wrap(~intervention)
```
